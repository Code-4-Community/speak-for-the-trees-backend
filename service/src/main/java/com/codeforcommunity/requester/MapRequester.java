...
public class MapRequester {
  ...
  private Future<Void> updateLayers(List<String> streetIds, BlockStatus updateTo, Future<String> tokenFuture) {
    /*
    var settings = {
    ...
    Future<Void> updateFuture = tokenFuture.compose(tokenString -> {
      ...
      return Future.future(promise -> {
        client.postAbs("https://services7.arcgis.com/iIw2JoTaLFMnHLgW/ArcGIS/rest/services/boston_street_segments_1/FeatureServer/0/applyEdits")
              ...
              if (ar.succeeded()) {
                ...
                if (httpResponse.statusCode() == 200) {
                  ...
                  if (responseBody.containsKey("error") && responseBody.getJsonObject("error").getInteger("code") == 498) {
                    ...
                    // throw error
                    logger.error("Update street request responded with unrecognized response body: " + responseBody);
                  }
                } else {
                  logger.error("Update street response request responded with non-200 status code: " + httpResponse.statusCode());
                }
              } else {
                logger.atError().withThrowable(ar.cause()).log("Error sending update street request", ar.cause());
  ...
  private Future<String> updateToken() {
    ...
    */
    ...
    Future<String> newTokenFuture = Future.future(promise -> {
      client.postAbs("https://www.arcgis.com/sharing/rest/oauth2/token")
          .putHeader("content-type", "multipart/form-data")
          .sendForm(formData, ar -> {
            if (ar.succeeded()) {
              HttpResponse<Buffer> httpResponse = ar.result();
              ...
              if (httpResponse.statusCode() == 200) {
                ...
                if (responseBody.containsKey("access_token")) {
                  ...
                  logger.error("ArcGIS token request responded with unrecognized response body: " + responseBody);
                }
              } else {
                logger.error("ArcGIS token request responded with non-200 status code: " + httpResponse.statusCode());
              }
            } else {
              logger.atError().withThrowable(ar.cause()).log("Error sending ArcGIS token request");
              promise.fail(ar.cause());
            }
          });
    });
